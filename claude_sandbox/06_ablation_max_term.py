...
if __name__ == "__main__":
    ...
    omega_with = train_pinn(t_norm, gyr, dt_jnp, use_max_term=True)
    omega_without = train_pinn(t_norm, gyr, dt_jnp, use_max_term=False)
    ...
    for case_idx, (omega_pred, title_suffix) in enumerate([
        ...
        for i in range(3):
            ax = axes[i, case_idx]
            ax.plot(t_np, gyr_np[:, i], '-', color=colors_meas[i], linewidth=1, alpha=0.8, label=f'Measured')
            ax.fill_between(t_np, gyr_np[:, i] - ENVELOPE, gyr_np[:, i] + ENVELOPE,
                             color=colors_meas[i], alpha=0.15, label=f'±{ENVELOPE} envelope')
            ax.plot(t_np, omega_pred[:, i], '--', color=colors_pred[i], linewidth=2.5, label=f'PINN')
    ...
    plt.tight_layout()
    out_path = RESULTS_DIR / "segment_23_ablation_max_term.png"
